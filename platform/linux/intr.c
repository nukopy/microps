...
int intr_register_irq_entry(unsigned int irq,
        ...
        if (entry->irq == irq) {
            ...
            */
            ...
            ・既存の IRQ のフラグが `INTR_IRQ_SHARED` と完全に一致しているか
              - flags と INTR_IRQ_SHARED が一致している場合は XOR
            の結果は偽になる
    ...
    debugf("registered new IRQ entry: irq=%u, name=%s", entry->irq,
           entry->name);
        ...
        default:
            ...
            for (entry = irqs; entry; entry = entry->next) {
                ...
                if (entry->irq == (unsigned int)sig) {
                    debugf("call ISR: irq=%d, name=%s", entry->irq,
                           entry->name);
...
void intr_shutdown(void) {
    ...
    /* Q.
     なぜ以下のコードで割り込み処理スレッドが起動済みかどうか確認できるのか？
     ...
     * `pthread_create(&tid, …)` として `tid` を新しいスレッドの ID
     で上書きする。
     *
